...
bool ModuleAudio::Unload_effects(Mix_Chunk* track)
{
	...
	if (track == nullptr)
	{
		LOG("Cannot unload effects: track is null");
		return false;
	}

	// Iterate through the stored effects
	for (int i = 0; i < MAX_FX; ++i)
	{
		if (effects[i] == track)
		{
			// Remove the effect and free the associated memory
			Mix_FreeChunk(track);
			effects[i] = nullptr;
			last_effect = std::max(0, static_cast<int>(last_effect - 1)); // Ensure same type
			LOG("Successfully unloaded chunk track!\n");
			return true; // Return immediately after finding and removing the effect
		}
	}

	// If the effect is not found
	LOG("Effect not found in the list, cannot unload\n");
	return false;
}
